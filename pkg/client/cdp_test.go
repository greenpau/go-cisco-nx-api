...
func TestParseShowCDPNeighborTableJsonOutput(t *testing.T) {
	testFailed := 0
	...
	for i, test := range []struct {
		...
		cdpNeighborTable, err := NewCDPNeighborTableFromBytes(content)
		if err != nil {
			if !test.shouldErr {
			...
			if test.shouldErr {
				t.Logf("FAIL: Test %d: input '%s', expected to throw error, but passed: %v", i, test.input, cdpNeighborTable)
				testFailed++
				continue
			}
		}
