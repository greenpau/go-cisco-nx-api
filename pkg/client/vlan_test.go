...
func TestParseShowVlanJsonOutput(t *testing.T) {
	testFailed := 0
	...
	for i, test := range []struct {
		...
		vlans, err := NewVlansFromBytes(content)
		if err != nil {
			if !test.shouldErr {
			...
			if test.shouldErr {
				t.Logf("FAIL: Test %d: input '%s', expected to throw error, but passed: %v", i, test.input, vlans)
				testFailed++
				continue
			}
		}
		...
		if vlans != nil {
			if (len(vlans) != test.count) && !test.shouldFail {
				t.Logf("FAIL: Test %d: input '%s', expected to pass, but failed due to len(vlan) [%d] != %d", i, test.input, len(vlans), test.count)
				testFailed++
				continue
			}
		}
