...
func TestParseShowIPARPResultJsonOutput(t *testing.T) {
	...
	for i, test := range []struct {
	...
	}{
		{
			...
			exp: &IpArpResponseResult{
				Body: IpArpResultBody{TableVrf: []struct {
					RowVrf []struct {
						TableAdj []struct {
							RowAdj []struct {
								Flags      string "json:\"flags\" xml:\"flags\""
					...
					TimeStamp  string "json:\"time-stamp\" xml:\"time-stamp\""
					Incomplete string "json:\"incomplete,omitempty\" xml:\"incomplete,omitempty\""
				}{Flags: "", IntfOut: "Ethernet1/1.52", IPAddrOut: "89.1.3.10", Mac: "", TimeStamp: "PT4S", Incomplete: "true"}}}}, CntTotal: 7, VrfNameOut: "default"}}}}}, Code: "200", Input: "show ip arp ", Msg: "Success"},
			shouldFail: false,
			shouldErr:  false,
		},
	} {
		...
		iparp, err := NewIpArpResultFromBytes(content)
		//fmt.Printf("%#v\n", iparp) //DEBUG
		if err != nil {
			if !test.shouldErr {
				t.Logf("FAIL: Test %d: input '%s', expected to pass, but threw error: %v", i, test.input, err)
