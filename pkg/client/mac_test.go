...
func TestParseShowMacAddressTableJsonOutput(t *testing.T) {
	testFailed := 0
	...
	for i, test := range []struct {
		...
		{
			input:      "show.mac.address-table.1",
			exp:        &MacAddressTable{},
			count:      1,
			shouldFail: false,
			shouldErr:  false,
		...
		{
			input:      "show.mac.address-table.2",
			exp:        &MacAddressTable{},
			count:      1,
			shouldFail: false,
			shouldErr:  false,
		...
		macAddressTable, err := NewMacAddressTableFromBytes(content)
		if err != nil {
			if !test.shouldErr {
			...
			if test.shouldErr {
				t.Logf("FAIL: Test %d: input '%s', expected to throw error, but passed: %v", i, test.input, macAddressTable)
				testFailed++
				continue
			}
		}
